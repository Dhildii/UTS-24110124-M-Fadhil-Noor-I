        ...
        <div class="flex-row justify-center items-center mb-6">
            ...
            <div class="bg-white p-6 rounded-lg border-2 w-full mt-2">
                <div>
                    ...
                    <div>
                        <form action="{{ route('barangs.update', $barang->id) }}" method="POST" class="w-full max-w-lg">
                            @csrf
                            @method('PUT')
                            <div class="mb-4">
                                <label for="kode" class="block text-gray-700">Kode Barang</label>
                                <input type="text" value="{{ old('kode', $barang->kode) }}" name="kode" id="kode" class="w-full p-2 border rounded" required>
                            </div>
                            <div class="mb-4">
                                <label for="nama" class="block text-gray-700">Nama Barang</label>
                                <input type="text" value="{{ old('nama', $barang->nama) }}" name="nama" id="nama" class="w-full p-2 border rounded" required>
                            </div>
                            <div class="mb-4">
                                <label for="harga" class="block text-gray-700">Harga</label>
                                <input type="number" value="{{ old('harga', $barang->harga) }}" name="harga" id="harga" class="w-full p-2 border rounded" required>
                            </div>
                            <div class="mb-4">
                                <label for="stok" class="block text-gray-700">Stok</label>
                                <input type="number" value="{{ old('stok', $barang->stok) }}" name="stok" id="stok" class="w-full p-2 border rounded" required>
                            </div>
                            <div class="mb-4">
                                <label for="deskripsi" class="block text-gray-700">Deskripsi</label>
                                <textarea value="{{ old('deskripsi', $barang->deskripsi) }}" name="deskripsi" id="deskripsi" class="w-full p-2 border rounded"></textarea>
                            </div>
